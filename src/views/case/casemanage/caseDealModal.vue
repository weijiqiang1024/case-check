 ...
 * @Date: 2019-07-01 15:31:36 
 * @Last Modified by: weijq@cychina.cn (韦继强)
 * @Last Modified time: 2019-07-12 15:14:42
 * @Version:V1.0 
...
<template>
  <a-modal
    ...
    :cancelButtonProps="{ props: {disabled: isDisabled} }"
  >
    ...
    <div class="contentArea">
      ...
      <div class="baseInfo" :style="`margin-top:${disabled==false?14:4}px`">
        <div class="panel">
          ...
          <div class="panelContent">
            <base-info ref="baseInfo" :record="record" :disabled="disabled" :processPersonArr="processPersonArr"/>
          </div>
        </div>
      </div>
      ...
      <div class="dealInfo">
        <div class="panel">
          ...
          <div class="panelContent">
            <DealOptions ref="dealOptions" :record="record" :disabled="disabled" />
          </div>
        </div>
      </div>
    </div>
...
import DealOptions from "./dealOptions1";
...
export default {
  ...
  methods: {
    ...
    handleSubmit() {
      //提交方法
      //表单数据提交方法
      let dealInfo = this.$refs.dealOptions.getDealData();
      if (dealInfo) {
        this.handleCaseDealOk(dealInfo);
      } else {
        return false;
      }
    },
    handleCancel() {
      ...
      // this.$refs.dealOptions.resetFields();
    },
    getFlowLine(caseId) {
      if (!caseId) return;
